...
const ProductListingPage: React.FC<ProductListingPageProps> = ({
  ...
  const availableBrands = useMemo(() => {
    // Get brands from the base list (before filters) to show all options
    let source = products;
  ...
  return (
    <motion.section 
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
      className="py-12 bg-[#fafafa] min-h-screen"
    >
        ...
        <div className="mb-12">
            {view === 'category' ? (
                <div className="flex flex-col md:flex-row md:items-end justify-between border-b border-slate-200 pb-8">
                    <div>
                        <span className="text-slate-500 font-bold uppercase tracking-[0.2em] text-xs mb-2 block">
                ...
                <div className="border-b border-slate-200 pb-8">
                     <span className="text-slate-500 font-bold uppercase tracking-[0.2em] text-xs mb-2 block">Search Results</span>
                     <h1 className="text-4xl md:text-5xl font-black text-slate-900 tracking-tighter">"{globalSearchQuery}"</h1>
                     <p className="text-slate-500 mt-2">{filteredProducts.length} matches found</p>
                </div>
            )}
        </div>
          ...
          <div className="flex-grow">
            ...
            <div className="flex justify-between items-center mb-6">
              ...
              <div className="flex items-center space-x-2">
                 ...
                 <select 
                    ...
                    className="bg-transparent text-sm font-bold text-slate-900 border-none focus:ring-0 cursor-pointer pr-8 py-0"
                 >
            ...
            {filteredProducts.length === 0 && (
              <div className="py-32 text-center bg-white rounded-3xl border border-slate-100">
                ...
                <p className="text-slate-500">Try adjusting your filters or checking another category.</p>
                <button onClick={() => {
                    setSelectedBrands([]);
                    setMaxPrice(500000);
                    setLocalSearchQuery('');
